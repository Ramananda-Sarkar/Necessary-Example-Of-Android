package com.example.textdrawer;
...
public class MainActivity extends Activity implements Runnable {
	...
	private void loadDrawables() {
		/*
		 * Create a simple TextDrawable with all default settings. Text display
		 * settings will be pulled from the theme.
		 */
		...
		/*
		 * Create a second TextDrawable with a path applied. Applying a path
		 * negates implicit bounds measurement, so you must explicitly call
		 * setBounds() with a size that covers your path constraint.
		 */
		...
		/*
		 * Create a circular path on which to draw the text. It's best to keep
		 * all path values positive, so the circle is centered such that it's
		 * radius keeps the circle from going below (0, 0).
		 * 
		 * We are using complex units so the values scale appropriately to
		 * different displays
		 */
		Path p = new Path();
		int origin = getResources()
				.getDimensionPixelOffset(R.dimen.path_origin);
		int radius = getResources()
				.getDimensionPixelOffset(R.dimen.path_radius);
		...
		/*
		 * Create a compound drawable with TextDrawable and a shape. Since
		 * TextDrawable is a non-framework class, we can't do this purely in
		 * XML. The example also includes adding insets to the text over the
		 * shape.
		 */
		d = new TextDrawable(this);
		d.setText("LAYER\nTEXT");
		Drawable background = getResources().getDrawable(R.drawable.background);
		LayerDrawable container = new LayerDrawable(new Drawable[] {
				background, d });
		...
		/*
		 * Wrap a simple TextDrawable in a ClipDrawable to animate. One
		 * convenient advantage of ImageView is we can call setImageLevel() on
		 * the view itself to affect the Drawable content.
		 */
		d = new TextDrawable(this);
		...
		/*
		 * Construct a simple TextDrawable to act as a static prefix inside of
		 * an EditText element
		 */
		d = new TextDrawable(this);
		d.setText("SKU#");
		 ...
		 * as the left and right drawable instance. The instance is also
		 * clickable, so the drawable text colors will change with state.
		 */
		d = new TextDrawable(this);
		d.setText("TEXT\nDRAW");
