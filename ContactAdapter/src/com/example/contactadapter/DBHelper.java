...
public class DBHelper extends SQLiteOpenHelper {
	...
	public Cursor sortByName() {
		...
		Cursor cursor = db
				.rawQuery("SELECT * FROM people ORDER BY name;", null);
		cursor.moveToFirst();
		return cursor;
	}
	...
	public void addContact(String name, String phoneNumber) {
		...
		if (column_idx >= 0) {
			String query = "Select * from people where phone like '%"
					+ phoneNumber + "%'";
			subCursor = db.rawQuery(query, null);
			if (subCursor.getCount() <= 0) {
				String insert = ("INSERT INTO people (_id, name, phone) VALUES (NULL,'"
						+ name + "','" + phoneNumber + "');");
				db.execSQL(insert);
			}
		} else {
			...
			if (subCursor.getCount() <= 0) {
				String[] statements = {
						...
						"INSERT INTO people (_id, name, phone) VALUES (NULL,'"
								+ name + "','" + phoneNumber + "');" };
				for (String statement : statements) {
					db.execSQL(statement);
				}
